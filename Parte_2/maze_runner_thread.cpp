...
void walk(Position pos, bool* exit_found) {
    ...
    while (!valid_positions.empty()){
        ...
        steps.push(std::thread(walk, next_pos, exit_found));

    }

    while (!steps.empty()){
        std::thread t1 = std::move(steps.top());
        steps.pop();
        t1.join();
    }
    ...
    return; // Placeholder - substitua pela lógica correta
}
